...
{
    std::string robotIp = "192.168.1.6";
...
{
    double pointa[] = {20, 280, -60, 200};
    double pointb[] = {160, 260, -30, 170};
    ...
    {
        {
            ...
            if (hasNonZero)
            {
                ...
                bool rFinish = stateFinish(feedbackData.ToolVectorActual[3], pt.r);
                if (xFinish && yFinish && zFinish && rFinish)
                {
                    isStateFinish = true;
                }
    ...
    {
        {
            ...
            if (feedbackData.ErrorStatus && feedbackData.EnableStatus)
            {
                ...
                for (int i = 1; i < errorIdVec.size(); i++)
                {
                    ...
                    if (std::stoi(errorIdVec[i]) != 0)
                    {
                        ...
                        {
                            printf("Controller Alarm: %d, Error Code for Alarm: %s, %s\n", beanController.id,
                                   beanController.en.description.c_str(), beanController.en.description.c_str());
                        }
                        else
                        {
                            if (m_CErrorInfoHelper.FindServo(std::stoi(errorIdVec[i]), beanServo))
                            {
                                printf("Serve/Motor Alarm: %d, Error Code for Alarm: %s, %s\n", beanServo.id,
                                       beanServo.en.description.c_str(), beanServo.en.description.c_str());
                            }
                        }
                    }
                }
            ...
            else
            {
                ...
                {
                    std::vector<std::string> contiRecvVec = regexRecv(m_DobotMove.Continue());
                    if (!std::stoi(contiRecvVec[0]))
                    {
                        isStateFinish = true;
                        break;
